...
public class ParamFlowSlot extends AbstractLinkedProcessorSlot<DefaultNode> {
	...
	private static final Map<ResourceWrapper, ParameterMetric> metricsMap = new HashMap<ResourceWrapper, ParameterMetric>();
			...
			for (ParamFlowRule rule : rules) {
				...
				if (paramIdx < 0) {
					if (-paramIdx <= args.length) {
						...
						// illegal index, give it a illegal positive value,
						// latter rule check will pass
	 ...
	 *            resource to init
	 * @param index
	 *            index to initialize, which must be valid
	 */
