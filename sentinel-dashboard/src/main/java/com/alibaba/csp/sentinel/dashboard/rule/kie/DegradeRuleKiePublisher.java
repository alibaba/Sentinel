...
public class DegradeRuleKiePublisher implements RuleKiePublisher<List<DegradeRuleEntity>> {
    ...
    @Override
    public void update(String serverId, List<DegradeRuleEntity> entities) {
        ...
        entities.forEach(ruleEntity -> {
            ...
            DegradeRule degradeRule = ruleEntity.toRule();
            Optional<KieConfigResponse> response = kieConfigClient.updateConfig(url, degradeRule);
    ...
    @Override
    public void add(String serverId, List<DegradeRuleEntity> entities) {
        if(StringUtils.isEmpty(serverId) || Objects.isNull(entities)){
            throw new NullPointerException();
        }
        ...
        entities.forEach(ruleEntity -> {
            ...
            DegradeRule degradeRule = ruleEntity.toRule();
            Optional<KieConfigResponse> response = kieConfigClient.addConfig(url, "DegradeRule", degradeRule, kieConfigLabel);
            if(!response.isPresent()){
