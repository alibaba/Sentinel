...
上述命令中我们指定几个 JVM 参数，其中 `-Dserver.port=8080` 用于指定 Spring Boot 启动端口为 `8080`，  
`-Dauth.username=sentinel`、 `-Dauth.password=123456` 用于指定控制台登录的用户和密码分别为sentinel和123456，  
如果省略这2个参数，默认用户和密码均为sentinel，其余几个是 Sentinel 客户端的参数。  
