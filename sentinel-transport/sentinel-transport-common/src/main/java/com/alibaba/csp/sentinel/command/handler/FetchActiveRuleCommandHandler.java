...
public class FetchActiveRuleCommandHandler implements CommandHandler<String> {
    ...
    public CommandResponse<String> handle(CommandRequest request) {
        JsonSerializer serializer = JsonTransformerLoader.serializer();
        String type = request.getParam("type");
        if ("flow".equalsIgnoreCase(type)) {
            return CommandResponse.ofSuccess(serializer.serialize(FlowRuleManager.getRules()));
        } else if ("degrade".equalsIgnoreCase(type)) {
            return CommandResponse.ofSuccess(serializer.serialize(DegradeRuleManager.getRules()));
        } else if ("authority".equalsIgnoreCase(type)) {
            return CommandResponse.ofSuccess(serializer.serialize(AuthorityRuleManager.getRules()));
        } else if ("system".equalsIgnoreCase(type)) {
            return CommandResponse.ofSuccess(serializer.serialize(SystemRuleManager.getRules()));
        } else {
