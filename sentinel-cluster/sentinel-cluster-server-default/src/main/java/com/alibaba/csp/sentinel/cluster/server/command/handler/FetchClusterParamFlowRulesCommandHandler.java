...
public class FetchClusterParamFlowRulesCommandHandler implements CommandHandler<String> {
    ...
    public CommandResponse<String> handle(CommandRequest request) {
        ...
        if (StringUtil.isEmpty(namespace)) {
            return CommandResponse.ofSuccess(JsonTransformerLoader.serializer().serialize(ClusterParamFlowRuleManager.getAllParamRules()));
        } else {
            return CommandResponse.ofSuccess(JsonTransformerLoader.serializer().serialize(ClusterParamFlowRuleManager.getParamRules(namespace)));
        }
    }
}
