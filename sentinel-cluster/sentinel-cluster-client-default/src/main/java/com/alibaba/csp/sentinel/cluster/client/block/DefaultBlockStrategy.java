 ...
 */
package com.alibaba.csp.sentinel.cluster.client.block;
 ...
 */
public class DefaultBlockStrategy implements PrioritizedRequestBlockStrategy {
    @Override
    public boolean doWithBlockStrategy(FlowRule rule, Context context, DefaultNode node, int acquireCount, boolean prioritized) {
        if (!prioritized) {
            return false;
        }
        ...
        switch (strategy) {
            case RuleConstant.TRY_AGAIN_BLOCK_STRATEGY:
                return tryAcquireToken(rule, context, node, acquireCount, false);
            case RuleConstant.TRY_UNTIL_SUCCESS_BLOCK_STRATEGY:
                return tryAcquireToken(rule, context, node, acquireCount, true);
            default:
                return false;
        }
    }
    ...
    private static boolean tryAcquireToken(FlowRule rule, Context context, DefaultNode node, int acquireCount, boolean prioritized) {
        sleep(node, rule);
