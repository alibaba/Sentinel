...
/**
 * 从指定目录下找出所有的metric文件，并按照指定时间戳进行检索，参考{@link MetricSearcherDefaultImpl#find(long, int)}。
 * 会借助索引以提高检索效率，参考{@link MetricWriterDefaultImpl}；还会在内部缓存上一次检索的文件指针，以便下一次顺序检索时
 * 减少读盘次数。
 *
 * @author leyou
 */
public class MetricSearcherDefaultImpl implements MetricSearcher {
     ...
     */
    public MetricSearcherDefaultImpl(String baseDir, String baseFileName) {
     ...
     * @param charset
     */
    public MetricSearcherDefaultImpl(String baseDir, String baseFileName, Charset charset) {
        if (baseDir == null) {
    ...
    public synchronized List<MetricNode> find(long beginTimeMs, int recommendLines) throws Exception {
        List<String> fileNames = MetricWriterDefaultImpl.listMetricFiles(baseDir, baseFileName);
        int i = 0;
        ...
        for (; i < fileNames.size(); i++) {
            ...
            long offset = findOffset(beginTimeMs, fileName,
                MetricWriterDefaultImpl.formIndexFileName(fileName), offsetInIndex);
            offsetInIndex = 0;
            if (offset != -1) {
