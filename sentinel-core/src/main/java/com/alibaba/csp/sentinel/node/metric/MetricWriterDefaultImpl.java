 ...
 * @author leyou
 */
public class MetricWriterDefaultImpl implements MetricWriter {
    ...
    public MetricWriterDefaultImpl(long singleFileSize) {
        this(singleFileSize, 6);
    }

    public MetricWriterDefaultImpl(long singleFileSize, int totalFileCount) {
    ...
    static List<String> listMetricFiles(String baseDir, String baseFileName) throws Exception {
        ...
        for (File file : files) {
            if (file.isFile()
                ...
                && !file.getName().endsWith(MetricWriterDefaultImpl.METRIC_FILE_INDEX_SUFFIX)
                && !file.getName().endsWith(".lck")) {
                list.add(file.getAbsolutePath());
            }
        }
        Collections.sort(list, MetricWriterDefaultImpl.METRIC_FILENAME_CMP);
        return list;
    }
