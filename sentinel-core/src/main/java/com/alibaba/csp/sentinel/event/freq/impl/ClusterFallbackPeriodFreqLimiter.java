...
/**
 * Base on rule id and resource to finish frequency limit, event should be class or subclass of {@link SentinelRuleEvent}.
 *
...
public class ClusterFallbackPeriodFreqLimiter extends PeriodFreqLimiter {
    ...
    protected String getLimitDimensionKey(SentinelEvent event) {
        if (event instanceof ClusterFallbackEvent) {
            ClusterFallbackEvent fallbackEvent = (ClusterFallbackEvent) event;
            return fallbackEvent.getResourceName() + ":" + fallbackEvent.getRule().getId();
        }
