...
final class SingleParameterInjector<T> {
   ...
   private final com.google.inject.spi.Dependency<T> dependency;

   private final Object source;

   private final InternalFactory<? extends T> factory;

   SingleParameterInjector(com.google.inject.spi.Dependency<T> dependency, BindingImpl<? extends T> binding) {
   ...
   T inject(InternalContext context) throws InternalProvisionException {
      com.google.inject.spi.Dependency<T> localDependency = dependency;
      com.google.inject.spi.Dependency previous = context.pushDependency(localDependency, source);
